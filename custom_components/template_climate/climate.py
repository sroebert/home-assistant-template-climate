...
class TemplateClimate(TemplateEntity, ClimateEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._load_config(hass, config)
        self._load_config_scripts(hass, config)
        self._init_optimisitic_state()
    ...
    def _load_config(self, hass: HomeAssistant, config: ConfigType) -> None:
        """Initialize the values of the climate."""
    ...
    def _load_config_script(
        ...
        if action := config.get(action_key):
            if feature:
                self._attr_supported_features |= feature

            return Script(hass, action, name, DOMAIN)

        return None

    def _load_config_scripts(self, hass: HomeAssistant, config: ConfigType) -> None:
        """Load scripts from config."""
        self._set_humidity_script = self._load_config_script(
            hass, config, CONF_SET_HUMIDITY_ACTION, ClimateEntityFeature.TARGET_HUMIDITY
        )
        self._set_hvac_mode_script = self._load_config_script(
            hass, config, CONF_SET_HVAC_MODE_ACTION
        )
        self._set_swing_mode_script = self._load_config_script(
            hass, config, CONF_SET_SWING_MODE_ACTION, ClimateEntityFeature.SWING_MODE
        )
        self._set_fan_mode_script = self._load_config_script(
            hass, config, CONF_SET_FAN_MODE_ACTION, ClimateEntityFeature.FAN_MODE
        )
        self._set_preset_mode_script = self._load_config_script(
            hass, config, CONF_SET_PRESET_MODE_ACTION, ClimateEntityFeature.PRESET_MODE
        )
        self._set_temperature_script = self._load_config_script(
            hass, config, CONF_SET_TEMPERATURE_ACTION, self._set_temperature_features
        )

    def _init_optimisitic_state(self) -> None:
    ...
    @property
    def _set_temperature_features(self) -> ClimateEntityFeature:
    ...
    def _update_min_temp(self, temp: Any) -> None:
        if temp is not None and temp not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse min temperature from %s", temp)
    ...
    def _update_max_temp(self, temp: Any) -> None:
        if temp is not None and temp not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse max temperature from %s", temp)
    ...
    def _update_current_temp(self, temp: Any) -> None:
        if temp is not None and temp not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse temperature from %s", temp)
    ...
    def _update_current_humidity(self, humidity: Any) -> None:
        if humidity is not None and humidity not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse humidity from %s", humidity)
    ...
    def _update_min_humidity(self, humidity: Any) -> None:
        if humidity is not None and humidity not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse min humidity from %s", humidity)
    ...
    def _update_max_humidity(self, humidity: Any) -> None:
        if humidity is not None and humidity not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse max humidity from %s", humidity)
    ...
    def _update_target_humidity(self, humidity: Any) -> None:
        if humidity is not None and humidity not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse target humidity from %s", humidity)
    ...
    def _update_target_temp_high(self, temp: Any) -> None:
        if temp is not None and temp not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse temperature high from %s", temp)
    ...
    def _update_target_temp_low(self, temp: Any) -> None:
        if temp is not None and temp not in (STATE_UNKNOWN, STATE_UNAVAILABLE):
            try:
                ...
                _LOGGER.exception("Could not parse temperature low from %s", temp)

    @callback
    def _update_hvac_mode(self, hvac_mode_value: Any) -> None:
        if hvac_mode_value is not None and hvac_mode_value not in (
            STATE_UNKNOWN,
            STATE_UNAVAILABLE,
        ):
            try:
    ...
    def _update_preset_mode(self, preset_mode_value: Any) -> None:
        if preset_mode_value is not None and preset_mode_value not in (
            STATE_UNKNOWN,
            STATE_UNAVAILABLE,
        ):
    ...
    def _update_fan_mode(self, fan_mode_value: Any) -> None:
        if fan_mode_value is not None and fan_mode_value not in (
            STATE_UNKNOWN,
            STATE_UNAVAILABLE,
        ):
    ...
    def _update_swing_mode(self, swing_mode_value: Any) -> None:
        if swing_mode_value is not None and swing_mode_value not in (
            STATE_UNKNOWN,
            STATE_UNAVAILABLE,
        ):
    ...
    def _update_hvac_action(self, hvac_action_value: Any) -> None:
        if hvac_action_value is not None and hvac_action_value not in (
            STATE_UNKNOWN,
            STATE_UNAVAILABLE,
        ):
            try:
